...
int findIntersectionHelper(Node *one, Node *two)
{
    if (one->data == two->data)
    {
        return one->data;
    }
    findIntersectionHelper(one->next, two->next);
}
...
int findIntersection(linkedlist one, linkedlist two)
{
    ...
    if (one.size > two.size)
    {
        ...
        res = findIntersectionHelper(temp, two.head);
    }
    else
    {
        diff = two.size - one.size;
        // making the ends meet hehe
        temp = two.head;
        while (diff > 0)
        {
        ...
        res = findIntersectionHelper(one.head, temp);
    }
    return res;
}
